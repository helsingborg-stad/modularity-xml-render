...
class JsonParser extends React.Component {
    ...
    mapData(jsonData) {
        ...
        let items = this.getObjectProp(
            jsonData,
            fieldMap.itemContainer ? fieldMap.itemContainer.split('.') : []
        );
        ...
        items = items.map(item => ({
            ...
            heading: fieldMap.heading.map(heading => {
                ...
                value =
                    value && isDate(value) && heading.dateFormat === 'Y-m-d'
                        ? getDate(value)
                        : value;
                value =
                    value && isDate(value) && heading.dateFormat === 'Y-m-d H:i'
                        ? getDateTime(value)
                        : value;
                return (
            ...
            content: fieldMap.content.map(content => {
                ...
                value =
                    value && isDate(value) && content.dateFormat === 'Y-m-d'
                        ? getDate(value)
                        : value;
                value =
                    value && isDate(value) && content.dateFormat === 'Y-m-d H:i'
                        ? getDateTime(value)
                        : value;
                return {
