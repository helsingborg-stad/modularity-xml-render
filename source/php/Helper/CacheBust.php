...
namespace ModularityXmlRender\Helper;
...
class CacheBust
{
    ...
    {
        $jsonPath = MODULARITYXMLRENDER_PATH . apply_filters('ModularityXmlRender/Helper/CacheBust/RevManifestPath', 'dist/rev-manifest.json');

        if (file_exists($jsonPath)) {
            return json_decode(file_get_contents($jsonPath), true);
        } elseif (WP_DEBUG) {
            echo '<div style="color:red">Error: Assets not built. Go to ' . MODULARITYXMLRENDER_PATH . ' and run gulp. See '. MODULARITYXMLRENDER_PATH . 'README.md for more info.</div>';
        }
    }
}
